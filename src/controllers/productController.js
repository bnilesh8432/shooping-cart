...
const createProduct = async (req, res) => {
    try {
        ...
        const uniqueTitle = await productModel.findOne({
            title: title
        })
        if (uniqueTitle) {
            return res.status(400).send({
        ...
        if (!validator.isValid(currencyId)) return res.status(400).send({
            status: false,
            message: "currencyId required"
        })
        if (currencyId) {
            if (currencyId != "INR") return res.status(400).send({
                status: false,
                message: "currencyId should be only INR "
            })
            data['currencyId'] = currencyId
        }

        if (!validator.isValid(currencyFormat)) return res.status(400).send({
            status: false,
            message: "currencyFormat required"
        })
        if (currencyFormat) {
            if (currencyFormat != "₹") return res.status(400).send({
                status: false,
                message: "currencyFormat should be only ₹ "
            })
            data['currencyFormat'] = currencyFormat
        }

        if (!validator.isEmptyString(isFreeShipping)) return res.status(400).send({
            status: false,
            message: "if you want to update isFreeShipping then put somthing in string ..."
        })
        if (isFreeShipping) {
            isFreeShipping = isFreeShipping.trim()
            if (!["true", "false"].includes(isFreeShipping)) return res.status(400).send({
                status: false,
                message: "isFreeShipping should be only true or false"
            })
...
const getProduct = async function(req, res) {
    try {
        ...
        if (!validator.isEmptyString(size)) return res.status(400).send({
            status: false,
        ...
        if (!validator.isEmptyString(price)) return res.status(400).send({
            status: false,
            message: "plz enter price.."
        })
        if (price) {
            let j_price = JSON.parse(price)
            console.log(j_price)
            if (Object.keys(j_price).length == 0) return res.status(400).send({
                status: false,
                message: 'plz enter price fliter..'
            })
            if (j_price.priceGreaterThan) {
                filters['price'] = {
                    $gt: j_price.priceGreaterThan
                }
            }
            if (j_price.priceLessThan) {
                filters['price'] = {
                    $lt: j_price.priceLessThan
                }
            }
        ...
        if (priceSort) {
            if (!(priceSort == 1 || priceSort == -1)) return res.status(400).send({
                status: false,
                message: 'plz give correct value for sotring ex. for ascending:1 & descending :-1'
            })
...
const getProductById = async (req, res) => {
    try {
        ...
        let allProducts = await productModel.findOne({
            _id: Id,
            isDeleted: false
        }).select({
            deletedAt: 0
...
const updateProduct = async (req, res) => {
    try {
        ...
        if (Object.keys(productData).length < 1 && typeof files == 'undefined') {
            return res.status(400).send({
                status: false,
                message: "Plz enter atleast one fild to update"
            });
        }

        if (!validator.isEmptyString(title)) return res.status(400).send({
            status: false,
            message: "if you want to update title then put something in string ..."
        })
        if (title) {
        ...
        if (!validator.isEmptyString(description)) return res.status(400).send({
            status: false,
            message: "if you want to update description then put something in string ..."
        })
        if (description) {
        ...
        if (!validator.isEmptyString(price)) return res.status(400).send({
            status: false,
            message: "if you want to update price then put somthing in string ..."
        })
        if (price) {
            if (!/^\d*[0-9]\d*$/.test(price)) return res.status(400).send({
                status: false,
                message: "price should be only numbers"
            })
            updateData['price'] = price
        }

        if (!validator.isEmptyString(currencyId)) return res.status(400).send({
            status: false,
            message: "if you want to update currencyId then put somthing in string ..."
        })
        if (currencyId) {
            if (currencyId != "INR") return res.status(400).send({
                status: false,
        ...
        if (!validator.isEmptyString(currencyFormat)) return res.status(400).send({
            status: false,
            message: "if you want to update currencyFormat then put somthing in string ..."
        })
        if (currencyFormat) {
            if (currencyFormat != "₹") return res.status(400).send({
                status: false,
        ...
        if (!validator.isEmptyString(style)) return res.status(400).send({
            status: false,
            message: "if you want to update style then put somthing in string ..."
        })
        if (style) {
            updateData['style'] = style
        }

        if (!validator.isEmptyString(installments)) return res.status(400).send({
            status: false,
            message: "if you want to update installments then put somthing in string ..."
        })
        if (installments) {
            if (!/^\d*[0-9]\d*$/.test(installments)) return res.status(400).send({
                status: false,
                message: "installments should be only numbers"
            })
...
let deleteByIDProduct = async (req, res) => {
    try {
        ...
        const DELETE = await productModel.findByIdAndUpdate(productId, {
            $set: {
                isDeleted: true,
                deletedAt: Date.now()
            }
