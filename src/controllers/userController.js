...
const createUser = async function (req, res) {
    try {
        ...
        if (!validator.isValidRequestBody(data)) return res.status(400).send({ status: false, Message: "Invalid request parameters, Please provide user details" })
        ...
        if (!validator.isValid(fname)) return res.status(400).send({ status: false, message: "Provide the First Name " });
        if (!/^[a-zA-Z ]{2,30}$/.test(fname)) return res.status(400).send({ status: false, message: "Enter valid  fname" });

        if (!validator.isValid(lname)) return res.status(400).send({ status: false, message: "Provide the last Name " });
        ...
        if (!validator.isValidPhone(phone)) return res.status(400).send({ status: false, message: "phone should be a valid indian phone number" });

        let PhoneCheck = await userModel.findOne({ phone: phone.trim() });
        if (PhoneCheck) return res.status(400).send({ status: false, message: `This no ${phone} is already present` });

        if (!validator.isValid(email)) return res.status(400).send({ status: false, message: "Provide the EmailId " });
        if (!validator.isValidEmail(email)) return res.status(400).send({ status: false, message: "Provide the Valid EmailId " });

        let checkmail = await userModel.findOne({ email: email });
        if (checkmail) return res.status(400).send({ status: false, message: `${email} already exists` });

        if (!validator.isValid(password)) return res.status(400).send({ status: false, message: "Provide the Password " });
...
const getUser = async function (req, res) {
    try {
        ...
        if (!validator.isValidObjectId(userId)) return res.status(400).send({ status: false, message: "enter valid user Id ..." })

        const user = await userModel.findOne({ _id: userId })
        if (!user) return res.status(404).send({ status: false, message: "userID dosen't exist" })
