    ...
    try {
        ...
        if (status || status == "") {
            if (!validator.isValid(status)) {
                ...
                return res.status(400).send({ status: false, message: "STATUS ONLY BE pending completed cancelled" })
            }
        }
        if (cancellable || cancellable === "") {
            if (cancellable == "true" || cancellable == "false") {
                ...
                return res.status(400).send({ status: false, message: "CANCELLABLE CAN ONLY BE THE TRUE AND FALSE" })
            }
        }
    ...
    try {
        ...
        if (status) {
            ...
            if (order.status == "canceled" || order.status == "completed") {
                return res.status(400).send({ status: false, message: `This order is alraedy ${order.status}` })
            }
