...
const createUser = async function (req, res) {
    try {
        ...
        if (!validator.isValidRequestBody(data) && typeof files == 'undefind') {
...
const userLogin = async (req, res) => {
    try {
        ...
        if (!validator.isValidRequestBody(data)) {
            return res.status(400).send({ status: false, message: "please enter data in reqbody" })
        }

        if (!email) {
            return res.status(400).send({ status: false, message: "email not present" })
        }
        if (!password) {
            return res.status(400).send({ status: false, message: "password not present" })
        }
        ...
        if (!User) {
            return res.status(404).send({ status: false, message: "user not found with this email id" })
        }

        let isValidPass = await bcrypt.compare(password, User.password)
        if (!isValidPass) {
            return res.status(404).send({ status: false, message: "enter correct password..." })
        }
...
const updateProfile = async function (req, res) {
    try {
        ...
        if (Object.keys(body).length == 0 && typeof file == 'undefined') {
            return res.status(400).send({ status: false, message: "plz enter atleast one field to update .." })
        }
        ...
        if (fname) {
            if (!validator.isValid(fname)) {
                return res.status(400).send({ status: false, message: "fname is don't vallid" })
            }
            if (!/^[a-zA-Z ]{2,30}$/.test(fname)) {
                return res.status(400).send({ status: false, message: "Enter valid  fname" });
            }
